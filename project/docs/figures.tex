...
\begin{figure*}[ht]
\centering
\includegraphics[height=4.5in, width=5.5in]{../plots/deng-figures/deng-main-v2.pdf}
...
(\textit{zygote/early2cell}) are essentially assigned to a single cluster, others generally have membership in multiple clusters. Each cluster is annotated by
the genes that are most distinctively expressed in that cluster, and by the gene ontology categories that these distinctive genes are most enriched for (see Table \ref{} for more extensive annotation results). See text for more discussion of biological processes driving these results.}
\label{fig:fig4}
\end{figure*}
