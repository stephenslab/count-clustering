...
For the thinned GTEx data the Structure plot for $K=15$ preserves most of the major features of the original analysis on unthinned data (\textbf{Supplementary Figure \ref{fig:suppfig}}). For the accuracy comparisons with distance-based methods, both methods suffer reduced accuracy in thinned data, but the model-based method retains its superior performance. For example, when thinning by a factor of 1,000  the success proportion in separating tissues is  $0.10$ for hierarchical clustering and $0.32$ for GoM model.
...
The code and scripts for reproducing results in this paper are available from \url{http://stephenslab.github.io/count-clustering/}.
