...
correctly partitioned samples into the two groups (See methods for details.) We found that the model-based method succeeded in $59 \%$ of the comparisons, compared with $29 \%$ for the distance-based method.
...
single-cell data from \cite{jaitin.xx} considered below. (See  \textbf{Fig \ref{fig:figS3}} [check!] and \textbf{Fig \ref{fig:figS2}} for results with other values of $p_{thin}$.) 
