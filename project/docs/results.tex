...
In brief, our approach to clustering starts by summarizing RNA-seq data by a table of counts $C_{N \times G} = (c_{ng})$, where $c_{ng}$ is the number of reads from sample $n$ mapped to gene (or transcript) $g$ \cite{Oshlack2010}.  We fit a GoM model to this table of counts. The GoM model assumes that 
...
Next we applied the GoM models to two single cell RNA-seq datasets, from Jaitin \textit{et al} \cite{Jaitin2014} and Deng \textit{et al} \cite{Deng2014}.  
...
For example, while all cells from the 16-cell stage have high memberships in the green cluster, cells coming from two of the embryos at this stage have memberships in both purple and yellow, while the other two have memberships only in yellow cluster. 

Finally, the results indicate a few single cells that appear to be outliers. For example, a cell from a 16-cell embryo is represented by the blue cluster - a cluster that represents cells at the zygote and early 2-cell stage. Also, a cell from an 8-stage embryo has strong membership in the purple cluster - a cluster that represents cells from the blastocyst stage. 
