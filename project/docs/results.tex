...
For the thinned GTEx data the Structure plot for $K=15$ preserves most of the major features of the original analysis on unthinned data (\textbf{Supplementary Figure \ref{fig:figS2}}). For the accuracy comparisons with distance-based methods, both methods suffer reduced accuracy in thinned data, but the model-based method retains its superior performance. For example, when thinning by a factor of $1,000$  the success proportion in separating tissues is  $0.10$ for hierarchical clustering and $0.32$ for GoM model.
...
Deng \textit{et al} collected single-cell expression data of mouse preimplantation embryos from the zygote to blastocyst stage \cite{Deng2014} and analyzed the trend of allele-specific expression in early embryo development. Here we analyze the combined counts of the two alleles. Visual inspection of the Principal Components Analysis in \cite{Deng2014} suggested 6-7 clusters, so we focus on the GoM model with $K=6$. 

The results (Figure \ref{fig:fig4}) clearly highlight the continuous pattern of grades of membership through early embryonic evelopment stages. Initially at the zygote stage, the embryos are represented by the blue cluster. Continuing on to the cleavage division stages, the grades of membership change to include a mixture of blue and magneta clusters at the mid 2-cell stage, a mixture of magneta and yellow clusters at the late 2-cell stage, and then a mixture of yellow and green at the 4-cell stage. Indeed, many of the clusters showed notable trends through the stages. For example, membership in the green cluster is non-existent in the early stages, but starting from the 4-cell stage, the green becomes more prominent in the 8-16 cell stages, drops substantially in the early and mid-blastocyte stages, and is essentially absent in the late blastocytes. More generally, the GoM results highlight biological similarity between embryos from adjacent stages than embryos from distant stages during preimplantation. 

Examining the clustering results by individual embryos highlights apparent embryo-level effects in the early stages (Figure \ref{fig:fig4}). For example, cells from the four embryos collected at the 16-cell stage are represented by a mixture of blue, green, yellow, and purple clusters. Two of the embryos at the 16-cell stage are driven by green and yellow clusters, while the other two  are driven by green and purple clusters. Finally, the results indicate a few samples that appear to be outliers. For example, a cell from a 16-cell embryo is represented by the blue cluster - a cluster that drives cells at the zygote and early 2-cell stage, and a cell from an 8-stage embryo is driven by the purple cluster - a cluster that drives cells at the blastocyst stage.

Enrichment analysis of the top driving genes in each cluster indicates that the grades of membership align with known biology during embryonic preimplantation (Table X). For example, the blue cluster was enriched with genes responsible for germ cell development (e.g., \textit{Bcl2l10}, \textit{Spin1}) and cleavage division (e.g., \textit{Tfgb2}, \textit{Pttg1}), which are essential to the zygote and 2-cell stages. Similarly, the green cluster was enriched with cytoskeletal genes (e.g., \textit{Fbxo15}) and cytoplasm genes (e.g., \textit{tceb1}, \textit{Hsp90ab1}), all of which are essential for compaction at the 8-cell stage and morula formation at the 16-cell stage. Furthermore, the results showed some clusters formed a biologically meaningful mixture. For example, cells in the blastocyst are represented by a mixture of orange and purple clusters. The orange cluster were enriched with genes involving in the formation of outer trophoblast cells (e.g., \textit{Tspan8}, \textit{Krt8}, \textit{Id2} \cite{Guo2010}), while the purple cluster were enriched with genes responsible for the formation of inner cell mass (e.g., \textit{Pdgfra}, \textit{pyy} \cite{Hou2007}). Reassuringly, these results of two underlying cell populations in the blastocyst are consistent with the known mammalian extra-embroynic lineages, the outer trophectoderm (TE) and internal primitive endoderm trophectoderm (PE), prior to implantation.
...
The codes and scripts for reproducing results in this paper are available from \url{http://stephenslab.github.io/count-clustering/}.
