---
title: "Testing squarem update in maptpx"
author: "Kushal K Dey"
date: "April 20, 2016"
output: output: 
  html_document:
...
In this script, we check how fast we can make the topic model algorithm if we get rid of the active set update. Here we shall consider both scenarios. On a simulated data, we will apply the topic model maptpx algorithm with the active set update and the topic model without the active set update and then check which method converges faster and what is the total time taken to reach convergence for either of the two methods. We would also like to figure out how many iterations are required by either of these methods before convergence is reached.
...
### active set update (every 2 iterations)
...
### active set update (every 5 iterations)

```{r echo=TRUE, eval=TRUE}
system.time(Topic_clus <- topics(counts, K=2, tol=0.001, method_admix=1,
                                 nbundles=10, use_squarem=TRUE));
...
### active set update (every 10000 iterations: essentially no update)
