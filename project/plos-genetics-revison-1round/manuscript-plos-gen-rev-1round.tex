...
similarities among samples approximately reflect an underlying latent ordering \cite{novembre.stephens.08, horseshoe}.
...
one another as well as competing. For example, $t$-SNE seems provides a much clearer indication of the cluster structure in the full GTEx data than
...
(Indeed the methods can be used in combination: both PCA and $t$-SNE can be used to visulalize the results of the GoM model, as an alternative or complement to the Structure plot.)
...
\paragraph*{S10 Fig.}
\label{figS10}
\bf{Sparse Factor Analysis loadings visualization of GTEx V6 tissue samples.} The colors represent the 20 different factors. The factor loadings are presented in a stacked bar for each sample. We performed SFA under the scenarios of when the loadings are sparse (left panel) and when the factors are sparse (right panel).}

\paragraph*{S11 Fig.}
\label{figS11}
\bf{Sparse Factor Analysis loadings visualization of GTEx brain tissue samples.} The colors represent the 6 different factors. The factor loadings are presented in a stacked bar for each sample. We performed SFA under the scenarios of when the loadings are sparse (left panel) and when the factors are
sparse (right panel).
