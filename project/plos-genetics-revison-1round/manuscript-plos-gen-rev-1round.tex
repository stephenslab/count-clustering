...
are a generalization of cluster models that allow each othersample to have membership in multiple clusters.
...
\subsection*{Visualization using other dimension reduction techniques}

GoM models have close connections with dimension reduction techniques such as sparse factor analysis, principal components analysis and t-Distributed Stochastic Neighbor Embedding (t-SNE). All of these methods have been routinely used to visualize the sample variation of RNA-seq data. See \cite{Engelhardt2010} for discussion of relationships among these methods in the context of inferring population genetic structure. We applied these related methods to the GTEx (\nameref{figS4}, \nameref{figS6}, \nameref{S8}) and the single-cell RNA-seq (\nameref{figS5}, \nameref{figS7}) data examples. The results revealed that the GoM models provide a more visually and biologically appealing summary of the data than the other dimension reduction techniques.

[visualize in high dimensions] In particular, for the GTEx data, the GoM models combined with Structure plot provides a convenient visualization of tissue samples in 20 dimensions corresponding to 20 clusters. While, PCA, MDS and t-SNE provide solutions that are difficult to visualize more than two dimensions at one time. Our example illustrates that the results produced by PCA and MDS are unable to distinguish majority of the tissues samples, except for whole blood and brain tissue samples. Meanwhile, although the t-SNE can accurately cluster most samples by their corresponding tissue types, these results provide little insight into tissue similarity. Applying SFA to the GTEx data (\nameref{figS7}) revealed several underlying clusters sharing across tissue samples, which (how about GOs? Are these clusters similar to the GoM results). However, the factor loadings are difficult to visualize to provide insight into sample structure. 

[tissue sharing] Comparing the methods for the GTEx brain tissue samples, we observed that PCA and MDS produce a solution in which most of the brain regions are not well distinguished except for Cerebellum and Cerebellar Hemisphere. Meanwhile, the GoM model revealed possibly four cell types underlying the brain subregions that are suggested as similar in PCA and MDS. As can be seen in Figure 1b, these brain subregions differ in their admixture of the four cell types (Royal Blue cluster, Turquoise cluster, Bright Red cluster, and Yellow Orange cluster). In addition, these brain subregions are similar in the admixture of the Royal Blue cluster, possibly suggest a pattern of sharing across brain regions. On the other hand, t-SNE fails to uncover a clustering pattern of the brain tissue samples (\nameref{S8}). For the GTEx data, t-SNE performs better than PCA and MDS and 

[continuous variation] Finally, applying to embryonic cells, the PCA and MDS recover a pattern of variation that is consistent with the developmental stages from which the embryos were collected. While, the results produced by t-SNE arrange cell samples by their embryonic stage by do not order the samples on a developmental continuum. In contract, the GoM results revealed a rich admixture of cell types which reflect important biological processes during embryonic preimplantation development. 

[Where does this go??]
In addition, we used the GTEx data to test whether the GoM model is more accurate in detecting substructure than distance-based clustering methods. Specifically, for each pair of tissues in the GTEx data we assessed whether or not each clustering method correctly partitioned samples into the two tissue groups (see Methods). The GoM model was substantially more accurate in this test, succeeding in $81 \%$ of comparisons, compared with $29 \%$ for the distance-based method (Fig~\ref{fig2}). This presumably reflects the general tendency for model-based
