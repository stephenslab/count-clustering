...
provide satisfactory summaries of the structure in these data (Supplementary Figures \ref{}): samples from quite different tissues are often super-imposed on one another in plots of PC1 vs PC2, and this issue is only partly alleviated by examining more PCs
(Supplementary Figure \ref{}). The hierarchical clustering provides perhaps better separation of tissues (Figure \ref{}), but producing
a clear (static) visualization of the tree is difficult with this many samples. By comparison $t$-SNE (Supplementary Figure \ref{}) and the GoM model (Figure \ref{}) both show a much clearer visual separation of samples by tissue, although they achieve this in very different ways.
...
Although it is not suprising that samples cluster by tissue, other results could have occurred. For example,
...
%red cluster (cluster 3), appears in all of Breast Mammary tissue, Adipose Subcutaneous and Adipose Visceral, pointing to their sharing of adipocytes. And the
 sky blue cluster (cluster 13), appears in testis, pituitary, and thyroid, reflecting shared homonal-related processes. At the same time, these tissues
are distinguished from one another both by their degree of membership in this cluster (testis samples have consistently stronger membership; thyroid samples consistently weaker), and by membership in other clusters. For example, pituitary samples, but not tesis or thyroid samples, have membership in the light purple cluster (cluster 2)
which is driven by genes related to neurons and synapsis. In the $t$-SNE results these three tissues simply cluster separately into visually distinct groups, with no indication that their expression profiles have something in common. Thus, although we find the $t$-SNE results visually attractive, this 2-dimensional projection contains less information than the Structure plot from the GoM (Fig \ref{}a), which uses color to represent the samples in a 20-dimensional space.
...
In comparison, applying PCA, MDS, hierarchical clustering and $t$-SNE to these brain samples reveals less of this finer-scale structure (Supplementary Figures \ref{}). Both PCA and MDS effectively cluster the samples into two groups -- those related to the cerebellum vs everything else.
...
Finally, during the blastocyst stages two new clusters (purple and orange in Fig~\ref{fig4}) dominate. The orange cluster is enriched with genes involved in the formation of trophectoderm (TE) (e.g., \textit{Tspan8}, \textit{Krt8}, \textit{Id2} \cite{Guo2010}), while the purple cluster is enriched with genes responsible for the formation of inner cell mass (ICM) (e.g., \textit{Pdgfra}, \textit{Pyy} \cite{Hou2007}).
...
chosen by them to help elucidate cell-fate decisions during early development of the mouse embryo. Still, the GoM model results (Figure \ref{supp})
do not support a clear division of these data into three distinct
groups (and neither do PCA or $t$-SNE; Figure \ref{fig:PCA-48genes}).
...
exactly the point of an exploratory analysis tool: to highlight issues for investigation, identify anomolies, and generate hypotheses for future testing.
...
both the pancreas (purple) and the adrenal gland (light green). This pattern can be seen in the Structure plot (Fig~\ref{fig1}) but not from other methods like PCA, $t$-SNE or hierarchical clustering (Figure \ref{}).
...
Finally, GoM models can be viewed as one of a larger class of ``matrix factorization" approaches to understanding structure in data, which also includes PCA, non-negative matrix factorization (NMF), and sparse factor analysis (SFA); see \cite{Engelhardt2010}. This observation raises the question of
whether methods like SFA might be useful for RNA-seq data. (NMF is so closely related to the GoM model that we do not discuss it separately here;
indeed, the GoM model is essentially NMF, because both grades of membership and expression levels within each cluster are required to be non-negative.)
Informally, one way to view SFA is as a generalization of the GoM model that allows samples to have {\it negative} memberships in some ``clusters" (actually, factors).
