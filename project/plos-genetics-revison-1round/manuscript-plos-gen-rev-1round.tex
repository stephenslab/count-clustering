...
PCA and MDS map multivariate gene expression data to linear dimensions that aim to maximize global variation between samples. The sample coordinates in PCA/MDS dimensions are traditionally presented in two-dimensional scatter plots. For the GTEx data, the PCA plot of the first versus the second dimension reveals that the whole blood samples are the most distinct among all the GTEx tissue samples. The higher PCA/MDS dimensions further reveal that xx tissue samples are separated from xx tissue samples (when plotting xx versus xx dimension) and xx tissue samples are separated from xx tissue samples (xx versus xx dimension) (TO DO: check PCA/MDS results to see if this is true and also up to which dimension). However, currently there does not exist a compact visualization for representing all the PCA/MDS dimensions for high dimensional data, except for series of bi-scatter plots.
...
analysis and (b) t-SNE (c) Multidimenstional scaling.} Samples of matching tissue types are
indicated by points of matching color.

\paragraph*{S5 Fig.}
\label{figS5}
{\bf Mouse embryo single cell sample visualization using (a) principle component
analysis and (b) t-SNE (c) Multidimensional scaling.} Single cell samples collected at the same
developmental stage are indicated by points of matching color.
...
\paragraph*{S7 Fig.}
\label{figS7}
{\bf Visualization of loadings from Sparse Factor Analysis on Deng et al single cell
