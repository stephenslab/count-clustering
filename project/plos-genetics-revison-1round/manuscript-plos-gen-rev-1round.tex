...
Second, because the GoM model allows partial membership in each cluster, it is better able to highlight partial similarities among distinct tissues.
...
In comparison, applying PCA, MDS, hierarchical clustering and $t$-SNE to these data reveals less of this finer-scale structure. Both PCA and MDS effectively cluster the samples into two groups -- those related to the cerebellum vs everything else -- although labelling the samples by tissue does indicate some finer-scale structure within the larger cluster. Hierarchical clustering also separates out the cerebellum-related tissues from the others, but again the tree format seems ill-suited to static 
visualization of over a thousand samples.  For reasons that we do not understand $t$-SNE performs poorly for these data: many samples are allocated to essentially identical locations, so overplotting obscures them.
