...
%    \end{figure*}
 ...
 \begin{figure*}[ht]
    ...
    \begin{subfigure}[t]{1\textwidth}
       \centering
        \includegraphics[height=2.5in, width=5.5in]{../plots/Zeisel/clus_10/struct_clus_10_cell_subtype.png}
        \vspace{-0.5in}  \qquad \qquad \caption{   $k=10$}
      \end{subfigure}
    \caption{ (\textit{top panel})  Structure plot of the1041 single cells for K=7 of the Jaitin \textit{et al} data \cite{Jaitin2014} arranged by the amplification batch. It is observed that the clustering patterns within each batch are similar and so, either the amplification batch is driving the clustering or it is confounded with the actual biological effects, making it difficult to interpret these clusters. (\textit{bottom panel}) Structure plot of all samples for $K=10$ of Zeisel et al data \cite{Zeisel2015}, arranged by the cell subtype labels that were determined by the authors using their BackSpin algorithm and subsequent marker gene annotations. While the cluster patterns within these cell subtypes are similar in general, the first few samples in Oligo6 show mild presence of red cluster and are markedly different from the rest of the samples in Oligo6 which do not show any trace of red cluster. These first few samples of Oligo6 look similar in pattern to some Oligo4 samples with mild red cluster presence. Oligo4 samples also shows some heterogeneity in terms of the proportion of red cluster present. This could either be due to misclassification of the Backspin algorithm, or due to technical effects unaccounted for (like plate effect, lane effect etc).}
 \label{fig:fig5}
 \end{figure*}
