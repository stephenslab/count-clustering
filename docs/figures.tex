...
%    \end{figure*}
 ...
 \begin{figure*}[ht]
    ...
    \begin{subfigure}[t]{1\textwidth}
       \centering
        \includegraphics[height=2.5in, width=4.5in]{../plots/deng_structure_4.png}
        \vspace{-0.5in}  \qquad \qquad 
      \end{subfigure}
    \caption{ (\textit{top panel})  Structure plot of the1041 single cells for K=7 of the Jaitin \textit{et al} data \cite{Jaitin2014} arranged by the amplification batch. It is observed that the clustering patterns within each batch are similar and so, either the amplification batch is driving the clustering or it is confounded with the actual biological effects, making it difficult to interpret these clusters. (\textit{bottom panel}) Structure plot of all samples for $K=4$ of Deng et al data \cite{Deng2014}, arranged by the preimplantation development phases of the cells and within each phase, arranged in the same order as in the data. Some developmental phases are represented by a single cluster, for example- \textit{zygote/early2cell}, \textit{mid/late2cell}, \textit{8cell/16cell} and \textit{lateblast}, while some developmental phases can be written as a mix of two clusters. For example, \textit{4cell} phase can be written as a mix of the cluster representing the \textit{mid/late2cell} and the \textit{8cell/16cell} phase}. 
 \label{fig:fig3}
 \end{figure*}
